...
var device = (function () {
    ...
    function getPolicyPayLoad(deviceId,category){
        ...
        var platforms = db.query(sqlscripts.devices.select13, deviceId);
        ...
        var obj = {};
        var upresult = db.query(sqlscripts.policies.select1, category,String(username));
        ...
        var ppresult = db.query(sqlscripts.policies.select2, category,platformName, common.getTenantIDFromEmail(username));
        ...
        var gpresult = db.query(sqlscripts.policies.select3, category,role, common.getTenantIDFromEmail(username));
    ...
    function sendMessageToAndroidDevice(ctx){
        ...
        var devices = db.query(sqlscripts.devices.select14, deviceId+"");
    ...
    function invokeInitialFunctions(ctx) {
        ...
        var tenantID = common.getTenantID();
        var devices = db.query(sqlscripts.devices.select15 ,ctx.deviceid);
    ...
    function sendMessageToIOSDevice(ctx){
        ...
        var devices = db.query(sqlscripts.devices.select16, ctx.deviceid+"");
        ...
        var users = db.query(sqlscripts.devices.select17, ctx.deviceid+"");
    ...
    function checkPendingOperations() {
        ...
        for(var i = 0; i < pendingOperations.length; i++) {
            ...
            var devices = db.query(sqlscripts.devices.select18, deviceId,tenantID);
    ...
    module.prototype = {
        ...
        sendToDevice: function(ctx){
            ...
            log.debug(ctx.operation);
            var devices = db.query(sqlscripts.devices.select19, ctx.deviceid+"");
            var platformID = devices[0].platform_id;
            if(platformID==1){
        ...
        getFeaturesFromDevice: function(ctx){
            ...
            var featureList = db.query(sqlscripts.devices.select20, stringify(deviceId));
        ...
        sendMsgToUserDevices: function(ctx){
            var tenantID = common.getTenantID();
            var device_list = db.query(sqlscripts.devices.select21, ctx.userid, tenantID);
            var succeeded="";
            var failed="";
        ...
        sendMsgToGroupDevices :function(ctx){
            ...
            for(var i = 0; i < userList.length; i++) {
                ...
                var result = db.query(sqlscripts.devices.select22, String(userList[i].email), tenantID);
        ...
        monitor:function(ctx){
            ...
            var result = db.query(sqlscripts.devices.select23);
        ...
        getCurrentDeviceState:function(deviceId){
            var tenantID = common.getTenantID();
            var result = db.query(sqlscripts.devices.select24, stringify(deviceId));
        ...
        registerAndroid: function(ctx){
            ...
            var platforms = db.query(sqlscripts.platforms.select8, ctx.platform);//from device platform comes as iOS and Android then convert into platform id to save in device table
            var platformId = platforms[0].id;

            var createdDate =  common.getCurrentDateTime();

            if(ctx.regid!=null){
                var result = db.query(sqlscripts.devices.select27, ctx.regid);
                ...
                if(result[0]==null){
                    ...
                    var devices = db.query(sqlscripts.devices.select27, ctx.regid);
                    var deviceID = devices[0].id;
        ...
        registerIOS: function(ctx){
            ...
            var platforms = db.query(sqlscripts.platforms.select8, ctx.platform);
            var platformId = platforms[0].id;
            var createdDate = common.getCurrentDateTime();

            var devicesCheckUDID = db.query(sqlscripts.device_pending.select3, ctx.auth_token);
            ...
            if(devicesCheckUDID != undefined && devicesCheckUDID != null && devicesCheckUDID[0] != undefined && devicesCheckUDID[0] != null){
                db.query(sqlscripts.device_pending.update2, tenantId, userId, platformId, stringify(ctx.properties), createdDate, ctx.vendor, ctx.udid, ctx.auth_token);
            } else {
                db.query(sqlscripts.device_pending.insert2, tenantId, userId, platformId, stringify(ctx.properties), createdDate, ctx.vendor, ctx.udid, ctx.auth_token);
            }
        ...
        getPendingOperationsFromDevice: function(ctx){
            ...
            var deviceList = db.query(sqlscripts.devices.select28, ctx.udid);
                ...
                if(pendingFeatureCodeList!=undefined && pendingFeatureCodeList != null && pendingFeatureCodeList[0]!= undefined && pendingFeatureCodeList[0]!= null){
                    ...
                    if(feature_code == "500P") {
                        ...
                        if(received_data == null || received_data == '') {
                            ...
                            db.query(sqlscripts.notifications.update1, stringify(arrEmptyReceivedData), id+"");
                        ...
                        for(var i = 0; i < received_data.length; i++) {
                            ...
                            if(received_data[i].status == "pending") {
                                ...
                                db.query(sqlscripts.notifications.update1, stringify(received_data), id+"");
                        ...
                        db.query(sqlscripts.notifications.update2, id+"");
                    }
        ...
        updateiOSTokens: function(ctx){
            ...
            var result = db.query(sqlscripts.device_pending.select4, stringify(ctx.deviceid));
            ...
            if(result != null && result != undefined && result[0] != null && result[0] != undefined) {
                ...
                var userResultExist = db.query(sqlscripts.devices.select29, ctx.deviceid);

                if(userResultExist != null && userResultExist != undefined && userResultExist[0] != null && userResultExist[0] != undefined) {

                    var devicePendingResult = db.query(sqlscripts.device_pending.select5, ctx.deviceid);
                    ...
                    db.query(sqlscripts.device_pending.update3, devicePendingResult.user_id);
                    ...
                    db.query(sqlscripts.device_pending.update4, ctx.deviceid);
                }
        ...
        updateDeviceProperties:function(deviceId, osVersion, deviceName, wifiMac) {
            ...
            var deviceResult = db.query(sqlscripts.devices.select30, deviceId + "");
        ...
        saveiOSPushToken:function(ctx){
            ...
            if (ctx.pushToken != null || ctx.pushToken != undefined) {
                ...
                var result = db.query(sqlscripts.devices.select31, ctx.udid);
                if (result[0].count > 0) {

...
var policy = (function () {
    ...
    module.prototype = {
        ...
        addPolicy: function(ctx){
            var existingPolicies =  db.query(sqlscripts.policies.select8, ctx.policyName, common.getTenantID());
        ...
        getAllPoliciesForMAM:function(ctx){
            var result = db.query(sqlscripts.policies.select10, common.getTenantID());
            return result;
        },
        getPolicy:function(ctx){
            var result = db.query(sqlscripts.policies.select11, ctx.policyid, common.getTenantID());
            return result[0];
        },
        ...
        enforcePolicy:function(ctx){
            var policyId =  ctx.policyid;
            var policies = db.query(sqlscripts.policies.select11, String(policyId), common.getTenantID());
            var payLoad = parse(policies[0].content);

            var users1 = db.query(sqlscripts.policies.select12, String(policyId));

            for(var i = 0;i<users1.length;i++){
                var devices1 = db.query(sqlscripts.devices.select34, users1[i].user_id, common.getTenantID());
            ...
            for(var i = 0;i<platforms.length;i++){
                if(platforms[i].platform_id == 'android'){
                    var devices2 = db.query(sqlscripts.devices.select35, String(1), common.getTenantID());
                    ...
                    var devices3 = db.query(sqlscripts.devices.select36, String(1));
            ...
            for(var i = 0;i<groups.length;i++){
                ...
                for(var j=0;j<users2.length;j++){
                    var devices4 = db.query(sqlscripts.devices.select34, users2[j].username, common.getTenantID());
        ...
        getPolicyPayLoad:function(deviceId,category){
            ...
            var ppresult = db.query(sqlscripts.policies.select2, category,platformName);
            if(ppresult!=undefined && ppresult != null && ppresult[0] != undefined && ppresult[0] != null ){
                var policyPayLoad = parse(ppresult[0].data);
                return policyPayLoad;
            }

            var gpresult = db.query(sqlscripts.policies.select3, category,role);
